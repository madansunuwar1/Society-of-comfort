...
const Completed = () => {
  ...
  useEffect(() => {
    const fetchdata = async () => {
      const url = `https://residency.np/api/services/complete-service/${id}`;
      try {
          ...
          <div className="px-6 py-6 flex flex-col gap-3">
            <div className="flex bg-white rounded-lg shadow-md">
              <div className="flex flex-col gap-2 p-3 rounded-lg">
                <h1 className="text-[#38922d] font-bold">Task Completed</h1>
                <p className="text-xs text-bold">2022/07/08</p>
                <p className="h-auto my-auto text-xs">
                  Requested for a services to fix the pipes. the pipes in the
                  kitchen are jammed
                </p>
              </div>
              <img
                ...
                alt=""
                className="w-1/3 my-auto mx-2 rounded-md"
              ></img>
            </div>
          </div>
        ) : (
          <div className="flex justify-center my-6 mx-24 text-center">
            You havent completed any task
          </div>
        )}
      </div>
    </div>
